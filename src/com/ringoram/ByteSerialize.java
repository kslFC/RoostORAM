...
/**
 * BucketとBucketMetadataを通信可能なbyte列へ変換するクラス。
 *
 * サーバとの通信ではJavaオブジェクトをそのまま送らず、bucket id・metadata・bucket dataを
 * 固定順序で連結したbyte配列として送受信する。
 */
public class ByteSerialize {
	public byte[] metadataSerialize(BucketMetadata meta){
		// read_counter、meta_buf、valid_bitsを順番に連結する。
	...
	public BucketMetadata metadataFromSerialize(byte[] metaBytes){
		// metadataSerialize()と同じ順序でbyte列を読み戻す。
		int startIndex = 0;
	...
	public byte[] bucketSerialize(Bucket bucket){
		// bucket id、metadata、全block dataをまとめて1つのpayloadにする。
	...
	public Bucket bucketFromSerialize(byte[] serialized){
		// bucketSerialize()で作ったpayloadからBucketを復元する。
		int startIndex = 0;
